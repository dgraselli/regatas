...
describe('polar', () => {
  it('coincide en los puntos de la grilla', () => {
    // TWA 90 (índice 6), TWS 15 (índice 2) => 5.5
    expect(boatSpeed(DEFAULT_POLAR, 90, 15)).toBeCloseTo(5.5, 5);
  });

  it('interpola entre puntos', () => {
    const s = boatSpeed(DEFAULT_POLAR, 90, 12.5); // entre TWS 10 (5.0) y 15 (5.5)
  ...
  it('en la zona muerta la velocidad es ~0', () => {
    ...
    expect(boatSpeed(DEFAULT_POLAR, 40, 15)).toBe(0);
  });

  it('clampea fuera de rango', () => {
    expect(boatSpeed(DEFAULT_POLAR, 90, 100)).toBeCloseTo(5.7, 5); // tope TWS 25
    expect(boatSpeed(DEFAULT_POLAR, 90, 1)).toBeCloseTo(4.0, 5); // piso TWS 5
  });
